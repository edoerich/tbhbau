// tbhbau · JS comum a todas as páginas: moeda global, chip do save, menu, modal de item, order book ao vivo,
// tabelas ordenáveis e Pix. Sem dependências. Exposto em window.tbh pro avaliador.js.
(() => {
  ...
  let CUR = ls.get('cur') === 'usd' ? 'usd' : 'brl';
  let LANG = ls.get('lang') || 'pt'; // o site é PT; o avaliador detecta o idioma do navegador se o visitante nunca escolheu
  ...
  // Strings do modal (PT/EN); o resto do site é PT
  const T = {
    pt: { loading: 'carregando order book…', error: 'não foi possível carregar o order book agora. Tente de novo em instantes.', steam: 'ver na Steam ↗', page: 'página do item →',
      ...
      buy: 'Ordens de compra', sell: 'Ordens de venda', chart: 'Histórico — menor venda (R$)', pts: n => `${n} pontos`, empty: '📈 Coletando histórico… o gráfico aparece conforme os preços forem registrados.', live: 'ao vivo', copied: 'Copiado ✓', copy: 'Copiar código Pix' },
    en: { loading: 'loading order book…', error: 'could not load the order book right now. Try again in a moment.', steam: 'view on Steam ↗', page: 'item page →',
      hint: 'Live order book from Steam · values in R$', lowest: 'Lowest ask', median: 'Median', vol: '24h volume', bid: 'Best bid', ask: 'Best ask', spread: 'Spread',
      buy: 'Buy orders', sell: 'Sell orders', chart: 'History — lowest ask (R$)', pts: n => `${n} points`, empty: '📈 Collecting history… the chart appears as prices get recorded.', live: 'live', copied: 'Copied ✓', copy: 'Copy Pix code' },
  };
  const t = (k, ...a) => { const v = (T[LANG] || T.pt)[k] ?? T.pt[k]; return typeof v === 'function' ? v(...a) : v; };
  ...
  async function itemUrl(hash) {
    ...
    return _idx[hash] ? `/item/${_idx[hash]}/` : null;
  }
  ...
  // ── Modal de detalhe do item (tabelas do mercado e do avaliador) ──
  ...
  async function openItemDetail(it) {
    ...
    $('#idetLink').textContent = t('steam');
    $('#idetHint').textContent = t('hint');
    const pg = $('#idetPage'); pg.classList.add('hidden'); pg.textContent = t('page');
  ...
  // Linhas/links com data-hash em tabelas de mercado abrem o modal (dados do snapshot)
  document.addEventListener('click', async e => {
    const el = e.target.closest('[data-hash][data-open]'); if (!el) return;
    if (e.target.closest('a')) return; // links normais seguem
    e.preventDefault();
  ...
  if (live) (async () => {
    ...
    try {
      ...
      set('bid', moneyBrl(ob.maxBuyCents)); set('ask', moneyBrl(ob.minSellCents)); set('buyCount', ob.buyCount);
      ...
      if (full && points.length >= 2) full.innerHTML = chartHtml(points, `Histórico completo — menor venda (R$)`);
  ...
  $$('table.sortable').forEach(tbl => {
    ...
    const sort = () => {
      if (!key) return;
      const rows = [...tbody.rows].filter(r => !r.classList.contains('more'));
      rows.sort((a, b) => {
        const av = a.dataset[key], bv = b.dataset[key];
        const an = parseFloat(av), bn = parseFloat(bv);
        const cmp = (!isNaN(an) && !isNaN(bn)) ? an - bn : (av == null ? -1 : bv == null ? 1 : String(av).localeCompare(String(bv)));
        return cmp * dir;
      });
    ...
    if (input) {
      ...
      const apply = () => {
        ...
        if (counter) counter.textContent = `${n} itens`;
      };
      input.addEventListener('input', apply); apply();
    }
  });
  // Lista filtrável simples (índice de itens)
  ...
  window.tbh = { get cur() { return CUR; }, get lang() { return LANG; }, setLang(l, persist = true) { LANG = l; if (persist) ls.set('lang', l); }, money, moneyBrl, netCents, volNum, fmtQty,
